...
details_file = open("network_details_whole_hand_layers.txt", "w+")
...
def loadImage(imageName):
    ...
    loadedImageNew = []
    for x in range(len(loadedImage)):
        newY = []
        for y in range(len(loadedImage[x])):
            newY.append([loadedImage[x][y]])
        loadedImageNew.append(newY)
    ...
    return np.array(loadedImageNew)
    ...
    def train(self, training_data_input, training_data_output, learning_rate, num_training_epochs):
        ...
        for epoch in range(num_training_epochs):
            ...
            for training_data_num in range(length_training_data):
                ...
                print(training_data_num, "/", length_training_data)
            print("")
            epoch_end_time = time.time()
            print("Epoch: ", epoch, end=", ")
            print("Learning rate: ", learning_rate, end=", ")
            print("Error: ", total_error_at_output, end=", ")
            runtime_count = epoch_end_time - epoch_start_time
            print(f"Runtime: ", runtime_count, " seconds")
...
training_data_input = []
for i in range(500):
    imageName = "handcoordstrainingdata/gesture" + str(i) + ".jpg"
...
for i in range(500):
    values_name = "handcoordstrainingdata/gesture" + str(i) + ".npy"
    new_values = np.load(values_name)
    new_output = []
    for i in range(21):
        new_output.append(new_values[0][i]) # append all x values
    for i in range(21):
        new_output.append(new_values[1][i]) # append all y values
    training_data_output.append(new_output)
# training_data_output is now array of arrays with 42 values in them representing [thumb_x, pinky_x...thumb_y,pinky_y...]
print("All training data setup.")

### set up neural network ###
model = Network([
    Input("sigmoid"),
    Convolution(20, (4,4,1), "sigmoid"), # num_filters, (filter_x x filter_y x num_channels)
    Maxpooling((3,3),2),
    Convolution(40, (2,2,20), "sigmoid"), # num_filters, (filter_x x filter_y x num_channels)
    Maxpooling((3,3),2),
    Flatten(),
    Hidden(2000, "sigmoid"),
    Output(42, "sigmoid")])
