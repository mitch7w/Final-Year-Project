...
for i in range(502):
    imageName = "/Users/mitch/Documents/University/Project/Project GitLab/prototypes/Neural network/five_gestures_new_webcam/gesture" + str(i) + ".jpg"
    training_data_input.append(loadImage(imageName))

training_data_output = []
for j in range(2):
  for i in range(51):
      training_data_output.append([1,0,0,0,0]) # one
  for i in range(50):
      training_data_output.append([0,1,0,0,0]) # two
  for i in range(50):
      training_data_output.append([0,0,1,0,0]) # three
  for i in range(50):
      training_data_output.append([0,0,0,1,0]) # four
  for i in range(50):
      training_data_output.append([0,0,0,0,1]) # five
...
x_train = np.array(training_data_input[:-50])
y_train = np.array(training_data_output[:-50])
x_test = np.array(training_data_input[-50:])
y_test = np.array(training_data_output[-50:])
...
model = keras.Sequential(
    [
        ...
        layers.Conv2D(16, kernel_size=(8,8), activation="relu"),
        layers.MaxPooling2D(pool_size=(3,3),strides=(4,4)),
