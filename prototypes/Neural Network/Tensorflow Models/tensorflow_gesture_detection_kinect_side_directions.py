...
def loadImage(background_image):
    ...
    for c_counter in range(0,80,5):
        for r_counter in range(0,60,5):
            ...
            if(region_sum > 30000): # if more white pixels here than average
...
for i in range(1,271):
    imageName = "/Users/mitch/Documents/University/Project/kinect_side_directions/gesture" + str(i) + ".jpg"
...
x_train = np.array(training_data_input[:-25])
y_train = np.array(training_data_output[:-25])
x_test = np.array(training_data_input[-25:])
y_test = np.array(training_data_output[-25:])
...
np.save("tensorflow_gesture_detection_kinect_side_directions/conv_weights", model.layers[0].get_weights()[0])
np.save("tensorflow_gesture_detection_kinect_side_directions/conv_biases", model.layers[0].get_weights()[1])

# print("Hidden Layer")
np.save("tensorflow_gesture_detection_kinect_side_directions/hidden_weights", model.layers[3].get_weights()[0])
np.save("tensorflow_gesture_detection_kinect_side_directions/hidden_biases", model.layers[3].get_weights()[1])
print(np.array(model.layers[3].get_weights()[0]).shape)

# print("Output Layer")
np.save("tensorflow_gesture_detection_kinect_side_directions/output_weights", model.layers[4].get_weights()[0])
np.save("tensorflow_gesture_detection_kinect_side_directions/output_biases", model.layers[4].get_weights()[1])
